...
class PurchaseOrder(models.Model):
    ...
    partner_id = fields.Many2one('res.partner', string='Vendor', required=True, states=READONLY_STATES,
                                 ...
                                 domain="[('contact_nature', '=', 'suppliers')]",
...
class SaleOrder(models.Model):
    ...
    def order_verification_periodicity(self):
        ...
        orders = self.env['sale.order'].search([('name', '=', 'Mon cron marche !!!')])
    ...
    partner_id = fields.Many2one(
        ...
        domain="[('contact_nature', 'in', ['prospect', 'customer'])]", )
    ...
    tax_exemption = fields.Boolean(string="Exonéré des taxes ?", default=False)
    tax_exemption_amount = fields.Monetary(string="Montant TVA exonéré")
    ...
    def _amount_by_group(self):
        for order in self:
            ...
            for line in order.order_line:
                ...
                taxes = line.tax_id.compute_all(price_reduce, quantity=quantity, product=line.product_id,
    ...
    #         subscription = self.env['sale.subscription'].search([('name', '=', vals['origin'])])
...
class SaleOrderLine(models.Model):
    ...
    def _compute_amount(self):
        ...
        for line in self:
            ...
            if line.order_id.tax_exemption:
                line.update({
    ...
    def create(self, vals_list):
        ...
        for line in lines:
            subscription = self.env['sale.subscription.line'].search([
                ('analytic_account_id.name', '=', line.order_id.origin),
                ('product_id', '=', line.product_id.id),
                ('quantity', '=', line.product_uom_qty),
                ('name', '=', line.name),
            ])
            if subscription:
                line.duration = subscription[0].duration

    def write(self, vals):
        for line in self:
            if line.discount and not line.env.user.has_group('sales_team.group_sale_salesman_all_leads'):
                if line.discount > 10 or line.discount > line.product_id.margin_rate:
                    raise ValidationError(
                        u"Vous ne pouvez faire une réduction supérieure à la marge produit ou à 10%"
                    )

            if line.product_id and line.product_id.lst_price and 'price_unit' in vals.keys() and not line.env.user.has_group(
                    'sales_team.group_sale_salesman_all_leads'):
                diff = (line.product_id.lst_price - line.price_unit) * 100 / line.product_id.lst_price
...
class AccountMove(models.Model):
    ...
    def create(self, vals_list: dict):
        ...
        for vals in vals_list:
            if 'invoice_origin' in vals.keys():
                order = self.env['sale.order'].search([('name', '=', vals['invoice_origin'])])
                if order:
                    vals.update({
                        'tax_exemption': order[0].tax_exemption,
                        'tax_exemption_amount': order[0].tax_exemption_amount,
                    })
...
class AccountMoveLine(models.Model):
    ...
    def create(self, vals_list):
        ...
        for line in lines:
            ...
            print(order_line)
            print(order_line.duration)
            if order_line:
                line.duration = order_line[0].duration
                line.quantity = order_line[0].product_uom_qty
...
class CalendarEvent(models.Model):
    ...
    is_robot = fields.Boolean(string="Créé par webmecanik")

    @api.model
    def create(self, vals_list):
        record = super(CalendarEvent, self).create(vals_list)
        return record
